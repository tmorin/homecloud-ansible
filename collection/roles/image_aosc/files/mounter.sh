...
function execute_mount() {
  ...
  echo "$offsetStartOneInBytes $sizeOneInByte"
  ...
  offsetEndTwo=$(fdisk -l "${IMAGE_FILE}" -o End | tail -n-1 | head -n+1)
  offsetStartTwoInBytes=$((offsetStartTwo * 512))
  sizeTwoInByte=$(((offsetEndTwo-offsetStartTwo) * 512))
  echo "$offsetStartTwoInBytes $sizeTwoInByte"

  deviceTwo=$(losetup -f "${IMAGE_FILE}" --offset ${offsetStartTwoInBytes} --sizelimit ${sizeTwoInByte} --show)
  sleep 1
  sync
