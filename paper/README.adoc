...
In the IT industry there is more efficient way to install and even manage infrastructure stuff: the infrastructure as code <<rhic>>.
...
Ansible projects can be stored in a Version Control System but also easily tested in virtual environments, especially within a <<g_continuous_integration, continuous integration>> process.
