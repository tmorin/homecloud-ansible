...
Ansible Roles are usually used in Ansible Playbook.
...
In order to mutate hosts using <<g_ssh, SSH>>, Ansible needs to resolve their _addresses_ (i.e. the IP).
Usually, local network rely on <<g_dhcp, DHCP>> to automatically assigned IPs to new devices.
In the `homecloud` context, this solution is not the efficient because the nodes' IPs could change over the time and then create issues with the building blocks operations.
Therefore, the best way is to be sure each node of the cluster has a static IP which will never ever change over the time.

By default, Armbian is configured to get an IP using DHCP, however a static IP can be easily configured from a configuration file.
